 ...
 *
 * Place at:  src/pages/ContactUs.jsx  (or wherever your contact page lives)
 *
 * Changes from original:
 *   • Form submits to Google Sheets (same sheet as the modal)
 *   • Enquiry-type tabs added to the contact page form too
 *   • Service / product dropdowns appear conditionally by tab
 *   • GOOGLE_SCRIPT_URL constant - replace with your deployed URL
 */
...
import { SERVICES_LIST, PRODUCTS_LIST } from '../components/ContactModal';

// ─── SAME URL as in ContactModal.jsx ─────────────────────────────────────────
...
// ─── Submit helper (shared with modal) ───────────────────────────────────────
const submitToSheets = async (payload) => {
  const params = new URLSearchParams(payload);
  const res = await fetch(`${GOOGLE_SCRIPT_URL}?${params.toString()}`, { method: 'GET' });
  ...
  return (
    <section className="relative min-h-[60vh] flex flex-col items-center justify-center overflow-hidden pt-24 pb-16"
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border text-xs font-body mb-8"
          style={{ border:'1px solid rgba(61,126,255,0.3)', background:'rgba(61,126,255,0.08)', color:'#60b4ff', ...fade(0.1) }}>
        ...
        <h1 className="font-display font-extrabold text-5xl sm:text-6xl lg:text-7xl leading-[1.06] tracking-tight mb-6"
          style={{ ...fade(0.22), color:'var(--color-text-heading)' }}>
          Let's Build the <span className="text-blue-gradient">Next</span><br/>
          <span className="text-blue-gradient">Generation</span> of Enterprise.
        </h1>
        <p className="font-body text-base sm:text-lg leading-relaxed max-w-2xl mx-auto"
          style={{ ...fade(0.38), color:'var(--color-muted)' }}>
          From supply chain resilience to agentic AI workflows, our team is ready to accelerate your journey.
        </p>
      </div>
...
const ContactFormSection = () => {
  ...
  const [form,         setForm]         = useState({ name:'', email:'', company:'', phone:'', service:'', product:'', message:'' });
  ...
  const inputStyle = {
    ...
    border:       `1px solid ${isDark ? 'rgba(255,255,255,0.10)' : 'rgba(61,126,255,0.2)'}`,
    ...
    <section className="relative py-16 lg:py-24 overflow-hidden" style={{ background:'var(--color-secondary)' }}>
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-start">
          ...
          {/* ── LEFT: Form ── */}
          <div ref={formRef} style={{ opacity:formInView?1:0, transform:formInView?'translateX(0)':'translateX(-40px)', transition:'all 0.8s cubic-bezier(.4,0,.2,1)' }}>
            <h2 className="font-display font-extrabold text-3xl sm:text-4xl mb-2" style={{ color:'var(--color-text-heading)' }}>
            ...
            <div className="flex flex-wrap gap-2 mb-6">
              {ENQUIRY_TABS.map(tab => (
                <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                  className="px-3.5 py-1.5 rounded-full text-xs font-display font-semibold transition-all duration-200"
                  style={{
                    background:  activeTab === tab.id ? '#3d7eff' : isDark ? 'rgba(24,28,42,0.8)' : 'rgba(61,126,255,0.06)',
                    border:      activeTab === tab.id ? '1px solid transparent' : `1px solid ${isDark ? 'rgba(255,255,255,0.08)' : 'rgba(61,126,255,0.2)'}`,
                    color:       activeTab === tab.id ? '#ffffff' : 'var(--color-muted)',
                    boxShadow:   activeTab === tab.id ? '0 0 12px rgba(61,126,255,0.4)' : 'none',
                  }}>
                  {tab.label}
                </button>
              ))}
            ...
            {status === 'success' ? (
              <div className="flex flex-col items-center justify-center py-16 rounded-2xl border text-center"
                style={{ background:isDark?'rgba(24,28,42,0.8)':'#ffffff', borderColor:'rgba(52,211,153,0.4)' }}>
                ...
                <h3 className="font-display font-bold text-xl mb-2" style={{ color:'var(--color-text-heading)' }}>Message Sent!</h3>
                <p className="font-body text-sm" style={{ color:'var(--color-muted)' }}>We’ll get back to you shortly.</p>
              </div>
            ) : (
              <form onSubmit={handleSubmit} className="space-y-5">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <select name="service" value={form.service} onChange={handleChange}
                        ...
                        style={{ ...inputStyle, ...focusStyle('service'), appearance:'none', cursor:'pointer' }}>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <select name="product" value={form.product} onChange={handleChange}
                        ...
                        style={{ ...inputStyle, ...focusStyle('product'), appearance:'none', cursor:'pointer' }}>
                ...
                <div>
                  ...
                  <textarea name="message" rows={5}
                    placeholder={activeTab === 'feedback' ? 'Share your experience or suggestions...' : 'Tell us about your project or challenges...'}
                    value={form.message} onChange={handleChange}
                    onFocus={()=>setFocused('message')} onBlur={()=>setFocused('')}
                    style={{ ...inputStyle, ...focusStyle('message'), resize:'vertical' }} />
                </div>

                {status === 'error' && (
                  <p className="text-sm font-body text-red-400">{errorMsg}</p>
                )}

                <button type="submit" disabled={status === 'loading'}
                  className="w-full flex items-center justify-center gap-2 py-4 rounded-xl font-display font-bold text-sm text-always-white transition-all duration-300 hover:shadow-[0_0_30px_rgba(61,126,255,0.5)] hover:scale-[1.01] active:scale-[0.99] disabled:opacity-60 disabled:cursor-not-allowed"
                  ...
                  {status === 'loading' ? (
                    ...
                    <>Send Message <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg></>
                  )}
                </button>
          ...
          {/* ── RIGHT: Global Presence ── (unchanged from original) */}
          <div ref={sideRef} className="space-y-6"
            style={{ opacity:sideInView?1:0, transform:sideInView?'translateX(0)':'translateX(40px)', transition:'all 0.8s cubic-bezier(.4,0,.2,1) 0.15s' }}>
            ...
            <div className="flex gap-2 flex-wrap">
              {offices.map((o, i) => (
                <button key={o.id} onClick={() => setActiveOffice(i)}
            ...
            {offices.map((office, i) => (
              <div key={office.id}
                ...
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h4 className="font-display font-bold text-base" style={{ color:'var(--color-text-heading)' }}>{office.title}</h4>
                </div>
                <div className="space-y-2.5">
                  <a href={office.mapUrl} target="_blank" rel="noopener noreferrer" className="flex items-start gap-2.5 group">
                    ...
                    <span className="font-body text-xs leading-relaxed group-hover:text-blue-accent transition-colors" style={{ color:'var(--color-muted)' }}>{office.address}</span>
                  </a>
                  <a href={`tel:${office.phone}`} className="flex items-center gap-2.5 group">
                    ...
                    <span className="font-body text-xs group-hover:text-blue-accent transition-colors" style={{ color:'var(--color-muted)' }}>{office.phone}</span>
                  </a>
                  {office.email && (
                    <a href={`mailto:${office.email}`} className="flex items-center gap-2.5 group">
                      ...
                      <span className="font-body text-xs group-hover:text-blue-accent transition-colors" style={{ color:'var(--color-muted)' }}>{office.email}</span>
                    </a>
                  )}
                </div>
              </div>
            ...
            <div>
              <p className="font-body text-[10px] font-bold tracking-widest uppercase mb-3" style={{ color:'var(--color-muted)' }}>
                Our Digital Footprint
              </p>
              <div className="relative rounded-2xl overflow-hidden border"
                style={{ borderColor:isDark?'rgba(255,255,255,0.08)':'rgba(61,126,255,0.15)', height:180 }}>
                <iframe src={offices[activeOffice].embedUrl} className="w-full h-full"
                ...
                <div className="absolute bottom-0 left-0 right-0 flex items-center justify-between px-4 py-2.5"
                  style={{ background:isDark?'rgba(13,15,22,0.88)':'rgba(255,255,255,0.9)', backdropFilter:'blur(8px)' }}>
                  <span className="font-body text-xs" style={{ color:'var(--color-muted)' }}>Global Connectivity Enabled</span>
                  <span className="flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-display font-bold"
            ...
            <div className="rounded-2xl border p-5"
              style={{ background:isDark?'rgba(24,28,42,0.8)':'#ffffff', borderColor:isDark?'rgba(255,255,255,0.08)':'rgba(61,126,255,0.18)' }}>
              <h4 className="font-display font-bold text-base mb-1" style={{ color:'var(--color-text-heading)' }}>Connect on Social</h4>
              <p className="font-body text-xs mb-4" style={{ color:'var(--color-muted)' }}>Stay updated with our latest insights on Agentic AI and Supply Chain resilience.</p>
              <div className="flex gap-3">
                {[
                  { href:'https://www.linkedin.com/company/acespire-consulting-solutions/', label:'LinkedIn', icon:<><path d="M16 8a6 6 0 016 6v7h-4v-7a2 2 0 00-2-2 2 2 0 00-2 2v7h-4v-7a6 6 0 016-6z"/><rect x="2" y="9" width="4" height="12"/><circle cx="4" cy="4" r="2"/></> },
                  { href:'https://twitter.com/Acespire_21',  label:'Twitter',  icon:<path fill="currentColor" d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/> },
                ].map(s => (
                  <a key={s.label} href={s.href} target="_blank" rel="noopener noreferrer"
                    ...
                    style={{ background:isDark?'rgba(61,126,255,0.12)':'rgba(61,126,255,0.08)', border:'1px solid rgba(61,126,255,0.3)', color:'#3d7eff' }}>
                    <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round">{s.icon}</svg>
                    {s.label}
                  </a>
                ))}
              </div>
  ...
  return (
    <section className="relative py-16 lg:py-24 overflow-hidden" style={{ background:'var(--color-primary)' }}>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        <div ref={ref} className="relative rounded-3xl overflow-hidden px-8 py-16 sm:px-14 sm:py-20"
          style={{ background:'linear-gradient(135deg,#1E3A8A 0%,#1E1B4B 100%)', opacity:inView?1:0, transform:inView?'scale(1)':'scale(0.96)', transition:'all 0.85s cubic-bezier(.4,0,.2,1)' }}>
          ...
          <div className="relative z-10 text-center">
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border text-xs font-body mb-7"
            ...
            <h2 className="font-display font-extrabold text-3xl sm:text-4xl text-always-white leading-tight mb-5 max-w-2xl mx-auto"
              style={{ opacity:inView?1:0, transform:inView?'translateY(0)':'translateY(20px)', transition:'all 0.75s ease 0.2s' }}>
              Ready to architect your <span style={{ color:'#60b4ff' }}>AI-driven</span> future?
            </h2>
            <p className="font-body text-base sm:text-lg mb-10 max-w-xl mx-auto leading-relaxed"
              style={{ color:'rgba(255,255,255,0.7)', opacity:inView?1:0, transition:'all 0.75s ease 0.35s' }}>
              Join 150+ enterprise leaders who have transformed their operations with Acespire's Digital Transformation framework.
            </p>
